...
//			Process mIpAddrProcess = runtime.exec("/system/bin/ping -c 1 8.8.8.8");
  ...
  * */
	 ...
	 */
	public static String parseObjToString(Object object, Class cls) {
		Gson gson = new Gson();
		String s = gson.toJson(object, cls);
		return  s;
	}
	public static Object parseStringToObj(String string, Class cls) {
		Gson gson = new Gson();
		Object obj = null;
		if (cls == Session.class){
			obj = gson.fromJson(string, Session.class);
		} else if (cls == AuditItem.class){
			obj = gson.fromJson(string, AuditItem.class);
		} else if (cls == AuditImage.class){
			obj = gson.fromJson(string, AuditImage.class);
		} else if (cls == GateImage.class){
			obj = gson.fromJson(string, GateImage.class);
		} else if (cls == LogItem.class){
			obj = gson.fromJson(string, LogItem.class);
		}
		return  obj;
	}
}
