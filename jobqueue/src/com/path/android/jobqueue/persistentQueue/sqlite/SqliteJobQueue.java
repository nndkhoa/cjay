...
public class SqliteJobQueue implements JobQueue {
	...
	@Override
	public List<JobHolder> findJobByGroupId(String groupId) {

		List<JobHolder> results = new ArrayList<JobHolder>();
		Cursor cursor = db.rawQuery(sqlHelper.FIND_BY_GROUP_ID_QUERY, new String[]{groupId});
		try {
			while (!cursor.moveToNext()) {
				JobHolder jobHolder = createJobHolderFromCursor(cursor);
...
//		jobHolder.setId(id);
		...
		// #tieubao: query and move all of relevant jobs
		List<JobHolder> list = findJobByGroupId(jobHolder.getGroupId());

		for (JobHolder job : list) {
			remove(job);
		}
