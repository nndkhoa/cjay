...
public class AuditorContainerActivity extends CJayActivity implements
	...
	void cameraClicked() {
		...
		mNewImageCount = 0;
		mNewImageUUID = "";
		CJayApplication.gotoCamera(this, mContainerSession,
	...
	public void onResume() {
		...
		Logger.Log("issue_report - " + mNewImageUUID + " - start - imageCount="
				+ mNewImageCount);
	...
	private void showReportDialogForNewImage() {
		if (isRunning() && mNewImageCount == 1
				&& !TextUtils.isEmpty(mNewImageUUID)) {
	...
	public void onEvent(CJayImageAddedEvent event) {
		Logger.Log("issue_report - " + event.getCJayImage().getUuid()
				+ " - cjayimage added - " + event.getTag());
		if (event.getTag().equals(LOG_TAG)) {
			...
			if (isRunning()) {
				...
				Logger.Log("issue_report - " + mNewImageUUID
						+ " - start - imageCount=" + mNewImageCount);
				showReportDialogForNewImage();
			}
		}
	}
